...
- [ ] **Step 1: Write the gate** — assertions:
  ...
  - **C2 design==runtime:** `gates spec` on a master spec that sets **BOTH `ElectrodeForce=2.5` AND `WeldCurrent=9`** (the weld-lobe policy present in the registry) → REJECT (exit 1); mirrors C1's runtime DENY. NOTE (from the A2 evaluator's fail-open-on-absent-antecedent semantics): the design-time spec MUST enumerate the antecedent member `ElectrodeForce` — a spec omitting it would not trigger the weld-lobe rule and would falsely PASS, breaking the design==runtime equivalence. A conformant C2 control: a master spec with `ElectrodeForce=4.0, WeldCurrent=9` → ACCEPT (antecedent false, lobe vacuous).
