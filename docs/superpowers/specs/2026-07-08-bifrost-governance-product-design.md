...
| **Mímir** | The **northbound feeder**: OT → canonical model → UNS. Keeper of the well of wisdom = the one who fills/guards the UNS (the single source of truth). A separate product. | No — separate track |
...
Bifrost governs; Heimdall (inside it) enforces at the write boundary; Mímir feeds the UNS from OT; Verðandi glues field integrations with durable saga. **Four distinct applications**, each consuming the others only through published data/wire contracts.
...
- **Mímir (future):** will read Bifrost's published definition format to model OT into conforming canonical UDTs and feed the UNS. Data contract only.
