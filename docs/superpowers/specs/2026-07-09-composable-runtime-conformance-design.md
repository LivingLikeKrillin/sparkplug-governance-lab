...
**IN:** `ConformanceEvaluator` (structural+type+envelope+cross-member+recipe, rich verdict, graceful-degrade) · `ConformancePolicy` governed artifact + a **published (documentary) `conformance-policy.schema.json`** + version-pinning + **provenance-sealed via the existing recipe provenance rails** (NOT a bespoke gate that validates a policy against the schema — see §7) · derivation of envelope from the model · `gates spec` rewired to the evaluator (`SpecConformanceChecker`/`SpecVerdict` subsumed) · `WeldControllerType`+instance+recipe in sim · **Heimdall runtime registry-loading** (load `UdtDefinition`+`ConformancePolicy` at startup) · **Heimdall ② conformance wiring** (after ① authz) · **`policy.json` migration** (numeric rules → `constraint{type}`, range removed) · **the Mixer `Recipe/Rpm` model+binding** required for ncmd no-regression · the killer gate.
...
- **The `conformance-policy.schema.json` is DOCUMENTARY, not gate-enforced.** Unlike the other published schemas it has no record↔schema parity test and no gate validates a policy JSON against it (the gates read policies via the lenient mapper). "Governed artifact" here means version-pinned + provenance-sealed + read by the evaluator — NOT schema-validated at a gate.
- **"Single governed source" = single ENFORCEMENT source, not single occurrence of the number.** Both boundaries now ENFORCE range from the one governed `udt` (policy.json's range is gone). But the sim still hard-codes its own OPC `EURange` (`[0,3000]` for Rpm, `[0,12]/…` for Weld) — that is the equipment's OPC self-description, reconciled to the governed model by the separate Mímir/spine derivation track, not a second enforced copy. Do not claim the number literally exists in exactly one place.
- **C5 "lifecycle" rides the existing RECIPE provenance rails.** The policy bytes are published/verified through the same content-hash flow as recipes (stored as `recipe/<policyRef>/…/recipe-setpoints.yaml`); this proves the provenance rails seal+verify the policy bytes, NOT a bespoke first-class conformance-policy provenance/SemVer type.
- **"design==runtime" is equivalence over the same evaluator + same governed policy, NOT the same inputs.** At design time the cross-member antecedent comes from the master spec's DECLARED setpoint; at runtime from a LIVE OPC read of the instance. Their agreement in the demo is contingent on the fixture author setting the spec value equal to the sim seed — which is correct real-world behavior (declared recipe vs measured value), but the guarantee is over rule logic + thresholds, not values.
- **Design-time completeness caveat:** a master spec that OMITS a cross-constraint's antecedent member does not trigger that rule (absent antecedent = not-triggered). Runtime is fail-closed on an unresolvable antecedent binding; design-time relies on specs enumerating antecedent members. ② also governs only BOUND setpoint nodes — coverage depends on every governed setpoint having a `nodeBinding`.
