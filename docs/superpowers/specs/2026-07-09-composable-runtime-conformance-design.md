...
Pure function. Inputs: the governed equipment `UdtDefinition`, the governed `ConformancePolicy`, the active recipe (for recipe-mode), and the candidate value(s). Derives and composes a small **closed** set of rule types:
- **type** — value type matches `member.type`.
- **envelope** — value within `member.range()` (derived from the model).
- **cross-member** — the policy's conditional constraints (e.g. `ElectrodeForce<3.0 ⇒ WeldCurrent≤8`); evaluated over the candidate + sibling member values.
- **recipe** (recipe-mode only) — value equals the active recipe's setpoint for the member (± a governed tolerance).

Returns a **rich verdict** (`ConformanceVerdict`): overall pass/fail + an ordered list of violations with exact reasons + the rule id that produced each. **Precedence** (memory constraint): authz-deny short-circuits before conformance (handled by the Heimdall caller); within conformance, all rules evaluate and violations **accumulate** (a value can fail envelope AND cross-member AND recipe — report all). The algebra is small and closed (a handful of rule types + a verdict model), NOT Turing-complete; adding a rule type is a bounded, engineer-owned change.

`SpecConformanceChecker` is **subsumed** by this evaluator (design-time gate uses the evaluator; keep `SpecVerdict`→`ConformanceVerdict` or adapt).

### 4.2 `ConformancePolicy` — the governed declarative config artifact (IAM analog)
A NEW first-class governed artifact (declarative JSON, published `conformance-policy.schema.json`): `crossConstraints[]` (conditional bound rules), `strictnessDial` (`envelope` | `recipe`, with `activeRecipe: ref@ver` when `recipe`), `nodeBindings[]` (`opcNodeId → {equipmentRef@ver, member}`). References the equipment model (and recipe). **Managed over its lifecycle through Bifrost's EXISTING control plane** — schema-gated, provenance-published, SemVer'd — exactly like recipes: author a new version → gate validates → provenance publishes → consumers pin/consume. This is the "IAM-like management layer"; the abstraction is the declarative policy schema + the control plane, not an authoring syntax. (An optional Kotlin type-safe builder that emits this JSON is a later authoring-ergonomics convenience — NOT built here.)

### 4.3 Wiring the two boundaries
- **Design-time (`gates spec`)** — evaluates a master recipe against the equipment model + conformance policy via the evaluator (envelope + cross-member; recipe-mode not applicable at authoring). Admits the recipe only if it conforms.
- **Runtime (Heimdall)** — ① authz (`policy.json`: target/command/principal, deny-by-default) unchanged = WHO/WHAT; ② NEW conformance: resolve the command's OPC node → governed member via the policy's `nodeBindings`, load the governed model + policy (+ active recipe if recipe-mode), evaluate, DENY with the exact conformance reason on failure. Precedence: authz-deny short-circuits before conformance. The `constraint{min,max,type}` block is **removed from `policy.json`** — range/type now derive from the model; `policy.json` becomes pure authz. (Secret-style deny-targets remain authz-deny-by-default = WHO, not conformance.)
...
**IN:** `ConformanceEvaluator` (small closed algebra, rich verdict) · `ConformancePolicy` governed artifact + `conformance-policy.schema.json` + schema-gate + provenance + SemVer · derivation of envelope from the model · `gates spec` rewired to the evaluator · Heimdall ② conformance wired (authz unchanged, `policy.json` range removed) · `WeldControllerType`+instance+recipe in sim · the killer gate.
