...
- `String entryHash(ActivationEvent e, String prevHash)` — `Sha256.hex(preimage(e, prevHash))`.
...
- `void append(ActivationEvent e)` — **now chains**: read the file's last non-blank line → parse as `LedgerEntry` → `prevHash = tail.entryHash()`; empty/absent file → `prevHash = GENESIS`. Compute `entryHash`. Append `LedgerEntry{e, prevHash, entryHash}` as one JSONL line. **O(1) append** — reads only the tail, not the whole chain (see §7 for the honest consequence).
...
- **LN3 — delete middle event detected:** remove one line from the middle of a ≥3-entry ledger. `verify-chain` → exit 1 `prev-link-broken` (the following entry's `prevHash` no longer matches its new predecessor). (Reorder is the same failure class; one assertion suffices.)
...
- **append trusts the tail** (O(1)); a pre-existing break is surfaced at verify/bind, not at append time (§7).
