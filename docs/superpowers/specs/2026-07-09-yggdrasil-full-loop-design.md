...
- **Runtime spec-conformance still not enforced.** Heimdall ② checks authz ("who") only; range-on-write is caught design-time by `gates spec`, never at the runtime write boundary. → the "composable runtime conformance" track.
