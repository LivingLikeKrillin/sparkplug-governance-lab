...
| Tail-truncation by a non-key-holder | Signed head | Recorded ledger tail must equal `head.tailEntryHash`; `head.seq` must match entry count; head signature must be valid + signer registered |
| Tail-truncation re-anchored by a lone registered insider | **NOT closed** (head is singly-signed — one key re-anchors, vs two to forge an entry) | Deferred: dual-signed (four-eyes) head OR external anchor — §9 |
...
- **Ledger + head simultaneous rollback is NOT detected — and the head is only SINGLY signed, so this is a one-key (not four-eyes) attack.** A signed head anchors the tail against truncation *by an attacker with no registered key*. But the head carries a single signature (`head.signedBy` need only be a *registered* principal — §4.7), whereas forging a ledger *entry* requires two distinct registered keys (cryptographic four-eyes). So a **single registered insider** can truncate the tail and re-anchor the head (rewrite `seq`/`tailEntryHash`, set `signedBy` = self, re-sign) with the one key they hold, and the verifier accepts it — no local trace. This asymmetry is deliberate but must not be oversold: T5 closes tail-truncation against *outsiders/non-key-holders*, not against a lone authorized insider. Truly closing the insider case needs **either a four-eyes (dual-signed) head OR an external append-only anchor** for the head — git commit, a notary/transparency log, or a hardware monotonic counter — a distinct future thread. (A dual-signed head would make re-anchoring need two colluding keys, restoring symmetry with entry forgery; it was considered and deferred to keep T5's head a single approver attestation.) This is the honest correction to T4 §9's "a signed head closes tail-truncation": a *local, singly-signed* head closes *outsider single-sided* truncation only.
