...
- `activatorSig`/`approverSig` are **nullable** — a legacy T4 line (no sig fields) deserializes with both null = "unsigned entry." Jackson: unknown-property tolerance already on; missing → null.
...
Functional interface: `Signatures sign(String entryHash)` returning `record Signatures(String activatorSig, String approverSig, PublicKey activatorPub, PublicKey approverPub)`, plus `String signHead(String preimage)` and `String approverPrincipal()`.
- `ActivationLedger.append(ActivationEvent e, LedgerSigner signer)`: when `signer == null` → **exact T4 behavior** (write unsigned `LedgerEntry`, no head). When non-null → compute entryHash, get signatures, assert `activatorPub ≠ approverPub` (cryptographic four-eyes; throws → activate fails closed, ledger untouched), write the signed line, then advance + write the signed head.
...
`ChainVerdict`-style verdict (reuse `ChainVerdict` or a parallel `SignedVerdict` with `intact/brokenIndex/rule`). `verify(String target)`:
...
3. Head: present; `head.tailEntryHash == last.entryHash`; `head.seq == history.size()-1`; head sig valid vs `AuthorizedKeys.forPrincipal(head.signedBy())`. Miss → `identity.head.missing` / `identity.head.tail-mismatch` / `identity.head.seq-mismatch` / `identity.head.sig-invalid`.
...
- **Require-signed switch:** a new `Config` flag `REQUIRE_SIGNED_ACTIVATION` (default: **on** for the T5 gate/target). When on, an unsigned legacy ledger fails closed (`identity.sig.missing`). When off, Heimdall falls back to T4 structural `verifyChain` only — this preserves the existing T4/T3 gates (`run-lineage-gate.sh`, `run-activation-gate.sh`, full-loop) which use unsigned ledgers, with **no changes to those scripts**.
...
| I4 | `activate --by alice --approved-by alice` OR same key file for both | REFUSED at activate (string SoD `activation.approval.self` and/or crypto `identity.four-eyes.same-key`), exit 1 |
